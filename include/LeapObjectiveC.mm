...
- (float)baseline
{
    return _interfaceDevice->baseline();
}
...
{
    return [NSString stringWithCString:_interfaceDevice->serialNumber().c_str()
                              encoding:[NSString defaultCStringEncoding]];
}

- (LeapVector *) position
{
    const Leap::Vector pos = _interfaceDevice->position();
    return [[LeapVector alloc]initWithX:pos.x y:pos.y z:pos.z];
}

- (LeapMatrix *)orientation
{
    Leap::Matrix m = _interfaceDevice->orientation();
    return [[LeapMatrix alloc] initWithLeapMatrix:&m];
}
...
- (int64_t)timestamp
{
    return _interfaceImage->timestamp();
}
